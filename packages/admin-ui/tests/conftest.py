...
# --- 라우트 레벨 테스트 하네스 --------------------------------------------------
# admin_app 은 ~2000줄 단일 모듈이라 블루프린트 분리 리팩터가 예정돼 있다. 그 전에
# "리팩터가 동작을 안 바꿨다"를 값싸게 검증할 수 있도록, 대표 데이터를 심고 모든
# GET 라우트가 200 을 주는지 + 필터/페이지네이션/카테고리 CRUD 가 도는지 보는
# 공용 픽스처를 여기 둔다. (import time 격리는 위쪽 모듈 레벨 코드가 이미 처리.)
