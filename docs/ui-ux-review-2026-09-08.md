...
**진행 결과 (2026-09-08)**: 사용자 "순차로 진행" 지시로 #15 제외 전부 적용 완료. 커밋
`5c81e0d`(P1) · `fc22c0f`(묶음 A) · `3973b48`(묶음 B) · `b3103de`(묶음 C).
#15(폰트 스케일)만 보류 — 부분 적용이 오히려 역효과라 별도 리팩터 대상.
...
`--fs-xs/sm/base/lg/xl` 토큰으로 12종 크기를 수렴하는 작업. **부분 적용하면 토큰 시스템과
산발적 리터럴이 공존해 오히려 더 뒤죽박죽**이 되므로, 전체를 한 번에 훑는 별도 리팩터로
진행하는 게 맞음. 회귀 위험(전 화면 미세 리사이즈) 대비 시각적 이득이 작아 우선순위 낮음.
